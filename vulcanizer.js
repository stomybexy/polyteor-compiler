        ...
        files.forEach((inputFile) => {
            ...
            if (process.env.VULCANIZE) {
                ...
                var rcfile = Async.runSync((done) => {
                    fsExtra.copy(path.dirname(inputFile.getPathInPackage()), path.dirname(path.join('.polyteor', ...inputFile.getPathInPackage().split(path.sep).slice(1))),
                        (err) => done(err))
                })
                if (rcfile.error) {
                ...
                let el = Utils.vulcanize(path.join('.polyteor', path.join(...inputFile.getPathInPackage().split(path.sep).slice(1))), {
                    inlineScripts: true
                });
                console.log('crisper ...');

                let jsFileName = path.basename(inputFile.getPathInPackage(), '.vul.html') + '.js';
                let out = Utils.crisper(el.result, jsFileName);
